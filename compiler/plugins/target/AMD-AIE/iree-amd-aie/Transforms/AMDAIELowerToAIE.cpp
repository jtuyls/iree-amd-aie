...
LogicalResult AIEDeviceBuilder::npuDmaCpyNdOpToAIE(
  ...
  if (dmaOp.getSource()) {
    ...
    // connection.
    AMDAIE::FlowOp flowOp = connectionOp.getFlowOp();
    if (flowOp && flowOp.getPacketId()) {
      pktInfoAttr = AIE::PacketInfoAttr::get(
          rewriter.getContext(),
          /*pkt_type*/ 0, /*pkt_id*/ flowOp.getPacketId().value());
    }
...
LogicalResult AIEDeviceBuilder::connectionToAIE(
  ...
  // // If the connection has been converted into a single packet flow op,
  // retrieve
  ...
  std::optional<uint8_t> packetId =
      flowOp ? flowOp.getPacketId() : std::nullopt;
