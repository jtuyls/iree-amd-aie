...
AIE::ObjectFifoCreateOp createObjectFifo(IRRewriter &rewriter,
  ...
  // std::optional<SmallVector<int64_t>> sourceShape =
  // getConstantIntValues(dmaOp.getSourceMixedSizes()); if (!sourceShape) return
  // dmaOp.emitOpError() << "non-constant source sizes found";
  // std::optional<SmallVector<int64_t>> targetShape =
  // getConstantIntValues(dmaOp.getTargetMixedSizes()); if (!targetShape) return
  // dmaOp.emitOpError() << "non-constant target sizes found";
  ...
  // if (srcMemSpaceInt == 1) {
  //   // Source on L2
  //   dtype = AIE::AIEObjectFifoType::get(sourceAccessType);
  // } else if (dstMemSpaceInt == 1) {
  //   // Destination on L2
  //   dtype = AIE::AIEObjectFifoType::get(targetAccessType);
...
LogicalResult accessOpToAIE(IRRewriter &rewriter,
  ...
  MemRefType newType = MemRefType::Builder(type).setMemorySpace(
      rewriter.getI64IntegerAttr(1));  // TODO
