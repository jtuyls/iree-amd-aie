...
//
scf::ForallOp getParentForallWithLocalMapping(Operation *op) {
  ...
  return getParentForallWithLocalMapping(forallOp);
}
...
void AMDAIELocalizeLogicalObjectfifoPass::runOnOperation() {
  ...
  moduleOp->walk([&](AMDAIE::LogicalObjectFifoFromMemrefOp op) {
    // The op is already within the locally mapped forall, so doesn't need any
    // change.
    if (getParentForallWithLocalMapping(op)) {
  ...
  for (AMDAIE::LogicalObjectFifoFromMemrefOp op : logicalObjectFifos) {
    // Get all local scf.forall operations containing users of this logical
    // objectFifo.
    llvm::SmallSetVector<scf::ForallOp, 4> localUsers;
    for (Operation *userOp : op->getUsers()) {
      if (scf::ForallOp forallOp = getParentForallWithLocalMapping(userOp)) {
        localUsers.insert(forallOp);
      }
    }

    // If no local scf.forall users found, we don't do any rewrite.
    ...
    for (scf::ForallOp forallOp : localUsers) {
      ...
      op->replaceUsesWithIf(clone, [&](OpOperand &use) {
        scf::ForallOp localParentForall =
            getParentForallWithLocalMapping(use.getOwner());
