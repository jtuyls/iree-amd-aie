...
struct CombineStridedOps
  ...
  LogicalResult matchAndRewrite(AMDAIE::DoublyStridedOpInterface op,
    ...
    // size_t sourceMaxNbDims = sourceDmaDimConfig.maxNbDims + 1;
    // DmaDimConfig targetDmaDimConfig(deviceModel, targetMemspaceInt.value());
    // size_t targetMaxNbDims = targetDmaDimConfig.maxNbDims + 1;
    // LLVM_DEBUG(llvm::dbgs() << "sourceMaxNbDims: " << sourceMaxNbDims << "\n");
    // LLVM_DEBUG(llvm::dbgs() << "targetMaxNbDims: " << targetMaxNbDims << "\n");
    ...
    bool areSourcesCombinable = areAccessPatternsCombinable(
        ...
        std::bind(&DmaDimConfig::exceedsNbDims, std::ref(sourceDmaDimConfig), _1));
    ...
    bool areTargetsCombinable = areAccessPatternsCombinable(
        ...
        std::bind(&DmaDimConfig::exceedsNbDims, std::ref(targetDmaDimConfig), _1));
