...
static OptionalParseResult amdaieTypeParser(DialectAsmParser &parser,
  ...
  if (name.equals("logicalobjectfifo")) {
    ...
    return result = AMDAIEObjectFifoType::get(elementType), success();
  }
  return {};
}
...
void AMDAIEDialect::printType(Type type, DialectAsmPrinter &printer) const {
  if (type.isa<AMDAIEObjectFifoType>()) {
    auto objectFifoType = type.cast<AMDAIEObjectFifoType>();
...
struct AMDAIEObjectFifoTypeStorage : TypeStorage {
  ...
  AMDAIEObjectFifoTypeStorage(MemRefType elementType) : elementType(elementType) {}
  ...
  /// `KeyTy`.
  static AMDAIEObjectFifoTypeStorage *construct(TypeStorageAllocator &allocator,
                                             const KeyTy &key) {
    // Allocate the storage instance and construct it.
    return new (allocator.allocate<AMDAIEObjectFifoTypeStorage>())
        AMDAIEObjectFifoTypeStorage(key);
  }
...
AMDAIEObjectFifoType AMDAIEObjectFifoType::get(MemRefType elementType) {
...
LogicalResult
AMDAIEObjectFifoType::verify(function_ref<InFlightDiagnostic()> emitError,
                             MemRefType elementType) {
  return success();
}
