...
FailureOr<AMDAIE::NpuCircularDmaCpyNdOp> FlowOp::getNpuCircularDmaCpyNdUser() {
...
// Build a NpuCircularDmaCpyNdOp with mixed static and dynamic entries and
// target and source BD IDs.
void NpuCircularDmaCpyNdOp::build(
    OpBuilder &b, OperationState &result, Value dma,
    ArrayRef<OpFoldResult> targetOffsets, ArrayRef<OpFoldResult> targetSizes,
...
void NpuCircularDmaCpyNdOp::build(OpBuilder &b, OperationState &result,
                                  Value dma, ArrayRef<int64_t> targetOffsets,
                                  ArrayRef<int64_t> targetSizes,
...
// Build a NpuDmaCpyNdOp with dynamic entries.
void NpuCircularDmaCpyNdOp::build(
    OpBuilder &b, OperationState &result, Value dma, ValueRange targetOffsets,
    ValueRange targetSizes, ValueRange targetStrides, ValueRange sourceOffsets,
...
struct NpuCircularDmaCpyNdOpReplacementBuilder {
  static void replace(NpuCircularDmaCpyNdOp dmaOp, PatternRewriter &rewriter,
    ...
    rewriter.replaceOpWithNewOp<NpuCircularDmaCpyNdOp>(
        ...
        srcMixedOffsets, srcMixedSizes, srcMixedStrides);
  }
};
}  // namespace
