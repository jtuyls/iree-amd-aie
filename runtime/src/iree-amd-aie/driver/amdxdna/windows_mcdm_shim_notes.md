...
runtime metadata format. The current clean path is for the compiler to emit
`amdaie-amdxdna-xclbin-fb`, keeping legacy `amdaie-pdi-fb` PDI-only while the
xclbin-backed format carries the AXLF/context bytes Windows MCDM requires and
lets Linux extract the selected raw PDI from the xclbin's `AIE_PARTITION`
section in userspace.
...
The runtime must not depend on XRT. Windows MCDM requires AXLF-shaped context
bytes for `D3DKMTCreateContextVirtual`, so the clean compiler/runtime split is:

1. Keep `amdaie-pdi-fb` as the PDI-only amdxdna artifact.
2. Use `amdaie-amdxdna-xclbin-fb` when the target needs Windows-compatible
   context packaging; this carries PDI, xclbin context bytes, control code,
   reconfiguration payloads, and host patch tables.
3. Keep `iree-aie-xclbinutil` and AXLF construction compile-time only, not in
   the runtime shim.
...
* `--iree-amdaie-device-hal=xrt` emits `amdaie-xclbin-fb`, the XRT backend's
  existing xclbin/control-code artifact.
* `--iree-amdaie-device-hal=amdxdna` normally emits PDI-only `amdaie-pdi-fb`.
* `--iree-amdaie-device-hal=amdxdna
  --iree-amdaie-amdxdna-emit-context-xclbin=true` emits
  `amdaie-amdxdna-xclbin-fb`, which keeps the amdxdna runlist/patch-table
  model and carries the AXLF/xclbin context bytes needed by Windows MCDM. The
  raw PDI for Linux-style native context creation is extracted from that xclbin
  instead of duplicated in the FlatBuffer.

The best packaging change for Windows MCDM is therefore not to make the
amdxdna runtime consume XRT's flatbuffer wholesale. Instead, use the dedicated
amdxdna xclbin schema. That preserves:

* Linux amdxdna behavior and tests: the existing PDI path remains valid.
...
production Windows MCDM HAL should keep the amdxdna executable model and use
the dedicated xclbin-backed amdxdna schema.
...
IREE amdxdna executable
  AXLF/xclbin context wrapper in amdaie-amdxdna-xclbin-fb
  pdi_index selecting an AIE_PARTITION record inside that xclbin
  runlists
  patch tables
        |
        v
IREE amdxdna command buffer
  same ERT_START_CU / ERT_START_NPU / ERT_CMD_CHAIN construction
        ...
        v
native backend
  Linux  : extract raw PDI from xclbin, create context from PDI
...
single-PDI xclbin and attached to the same entry in
`amdaie-amdxdna-xclbin-fb`. The runtime caches contexts by complete xclbin
image for Windows so the detail is hidden from users and repeated dispatches do
not need to recreate contexts.
...
runtime/src/iree-amd-aie/schemas/pdi_executable_def.fbs
  PDI-only legacy amdxdna executable schema

runtime/src/iree-amd-aie/schemas/amdxdna_xclbin_executable_def.fbs
  xclbin-backed amdxdna executable schema with AXLF/xclbin, per-entry PDI
  record indices, runs, payloads, and patch tables

runtime/src/iree-amd-aie/driver/amdxdna/executable_internal.h
  kernel params carry xclbin bytes and extracted PDI bytes when loaded from
  amdaie-amdxdna-xclbin-fb

runtime/src/iree-amd-aie/driver/amdxdna/native.h
  native create_context receives PDI plus xclbin bytes when available

runtime/src/iree-amd-aie/driver/amdxdna/device.cc
  context cache key uses extracted PDI plus kernel name for PDI-native drivers
  and complete xclbin image for xclbin-native Windows contexts, so one
  multi-PDI context can expose multiple CUs.
...
The compiler now fills the xclbin-backed schema for single-PDI context wrappers
when `--iree-amdaie-amdxdna-emit-context-xclbin=true` is set. For multi-PDI
command-chain workloads, the remaining packaging work is to share one
multi-PDI AXLF/xclbin wrapper across the entry points that must run in one
Windows context.

Validation status:

* `live802` validates the new `amdaie-amdxdna-xclbin-fb` VMFB on Windows MCDM
  for single-dispatch matmul correctness.
* `live803` and `live810` validate same-context command chaining with the new
  schema, including 1000 logical runs with correct output.
* `live804`/`live805` benchmark the same generated xclbin/control-code pair:
  XRT module-runlist measured `17.779 us/run` in that adjacent sample, while
  MCDM path-B command chaining measured `15.322 us/run`.
* `live806`/`live807` captured an initial non-chain timing sample on the same
  artifact: XRT module/reused-run measured `97.16 us/run`; MCDM non-chain
  path-B measured `194.413 us` mean submit/wait. Treat this as superseded by
  the stricter reused-command timing in `live820`..`live823`; it mixed a short
  XRT reused-run sample with a noisier MCDM benchmark shape.
* `live808` confirms legacy PDI-only VMFBs are intentionally rejected on
  Windows MCDM because the runtime no longer reads optional xclbin bytes from
  `amdaie-pdi-fb`.
* `live812`/`live813` repeat correctness on a freshly compiled xclbin-only XADX
  artifact after removing the duplicated raw-PDI pool: single-dispatch and
  1000-logical-run MCDM command-chain both return the expected all-16 matmul
  output.
* `live814`/`live815` benchmark the fresh 240-logical-run chain shape:
  adjacent XRT measures `13.3752 us/run`; MCDM measures `12.862 us/run`.
  `live818`/`live819` benchmark the fresh 1000-logical-run chain shape:
  adjacent XRT measures `13.7014 us/run`; MCDM measures `10.713 us/run`.
* `live816`/`live817` repeat the short-sample single-dispatch mismatch:
  XRT module reuse is `80.07 us/run`, MCDM non-chain path-B is `176.599 us`
  mean submit/wait. `live820`..`live823` correct the methodology by comparing
  XRT reused `xrt::run` against one prepared MCDM command submitted repeatedly
  through `--amdxdna_n_kernel_runs`, with 50 warmups skipped on both sides.
  In that apples-to-apples window, full matmul is XRT `186.835 us/run` versus
  MCDM `163.899 us/run`, with correct all-16 output; zero-instruction no-op is
  XRT `106.772 us/run` versus MCDM `108.288 us/run`, with correct all-zero
  output. The current single-submit focus is tail jitter and async/depth
  design, not a confirmed packet/schema overhead gap.
* `live824`/`live826` repeated the pure-XRT reused-run baseline immediately
  before testing the normal `iree-benchmark-module` path: full matmul measured
  `144.318 us/run`, no-op measured `95.612 us/run`. `live825`/`live827` then
  measured `iree-benchmark-module` with `amdxdna_n_kernel_runs=1`, Google
  Benchmark warmup, and 1000 timed exported-function iterations: full matmul
  reported `2.45 ms` real time, zero-instruction no-op reported `2.21 ms` real
  time. This is the HRX/IREE entrypoint benchmark shape, but it is not the KMT
  submit-path microbenchmark: it includes VM invocation, command construction,
  binding/output handling, synchronization, allocator/cache behavior, and
  benchmark harness overhead. Keep `amdxdna_n_kernel_runs` as a diagnostic
  repeat helper for isolating submit latency, not as the production HRX runtime
  API surface.
