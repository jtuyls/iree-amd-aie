...
static iree_status_t iree_hal_amdxdna_direct_command_buffer_update_buffer(
  ...
  shim_xdna::bo* target_device_buffer = iree_hal_amdxdna_buffer_handle(
...
static iree_status_t iree_hal_amdxdna_direct_command_buffer_copy_buffer(
  ...
  shim_xdna::bo* source_device_buffer = iree_hal_amdxdna_buffer_handle(
...
static iree_status_t iree_hal_amdxdna_direct_command_buffer_accumulate_chained(
  ...
  for (iree_host_size_t j = 0; j < bindings.count; ++j) {
    group.binding_bos.push_back(iree_hal_amdxdna_buffer_handle(
        iree_hal_buffer_allocated_buffer(bindings.values[j].buffer)));
  }
...
static iree_status_t iree_hal_amdxdna_submit_chain(
  ...
  if (chain_bytes > chain_bo->size()) {
    return iree_make_status(IREE_STATUS_RESOURCE_EXHAUSTED,
                            "amdxdna cmd-chain: %zu slots exceed exec buffer "
                            "(%zu > %zu bytes)",
  ...
  if (arg_total > kArgBoCeiling) {
    return iree_make_status(
        ...
        "amdxdna cmd-chain: %zu arg BOs exceeds shim ceiling %zu (chunk "
  ...
  if (int err = hwq->issue_command(chain_bo)) {
    return iree_make_status(
        IREE_STATUS_INTERNAL,
        "amdxdna ERT_CMD_CHAIN submit (EXEC_CMD ioctl) failed: errno %d", err);
  }
  int rc = hwq->wait_command(chain_bo, 0);
  if (rc < 0) {
    return iree_make_status(IREE_STATUS_INTERNAL,
                            "amdxdna ERT_CMD_CHAIN wait ioctl failed: errno %d",
                            -rc);
  }
  if (rc == 0) {
    return iree_make_status(IREE_STATUS_DEADLINE_EXCEEDED,
                            "amdxdna ERT_CMD_CHAIN of %zu slots timed out", n);
  }
  ...
  if (cp->state != ERT_CMD_STATE_COMPLETED) {
    return iree_make_status(
        IREE_STATUS_INTERNAL,
        "amdxdna ERT_CMD_CHAIN of %zu slots did not complete: ert state %u "
...
static iree_status_t iree_hal_amdxdna_direct_command_buffer_flush_chains(
  ...
  for (iree_hal_amdxdna_chain_group& group : groups) {
    for (size_t begin = 0;
      ...
      status = iree_hal_amdxdna_submit_chain(shim_device, group.hwq, group,
                                             begin, end);
    }
...
static iree_status_t iree_hal_amdxdna_direct_command_buffer_normal_run(
  ...
  for (int i = 0; i < n_kernel_runs; i++) {
    ...
    if (int err = hwq->issue_command(ebuf.get_exec_buf_bo())) {
      ...
      return iree_make_status(
          IREE_STATUS_INTERNAL,
          "amdxdna dispatch submit (EXEC_CMD ioctl) failed: errno %d", err);
    }
    int rc = hwq->wait_command(ebuf.get_exec_buf_bo(), 0);
    if (rc < 0) {
      IREE_TRACE_ZONE_END(z0);
      return iree_make_status(IREE_STATUS_INTERNAL,
                              "amdxdna dispatch wait ioctl failed: errno %d",
                              -rc);
    }
    if (rc == 0) {
      IREE_TRACE_ZONE_END(z0);
      return iree_make_status(IREE_STATUS_DEADLINE_EXCEEDED,
                              "amdxdna dispatch timed out");
    }
    ...
    if (ebuf.m_cmd_pkt->state != ERT_CMD_STATE_COMPLETED) {
      IREE_TRACE_ZONE_END(z0);
      return iree_make_status(IREE_STATUS_INTERNAL,
                              "amdxdna dispatch did not complete: ert state %u",
...
static iree_status_t iree_hal_amdxdna_direct_command_buffer_reconfigure(
  ...
  for (int i = 0; i < n_reconfigure_runs; ++i) {
    ...
    if (int err = hwq->issue_command(ebuf.get_exec_buf_bo())) {
      ...
      return iree_make_status(
          IREE_STATUS_INTERNAL,
          "amdxdna reconfiguration submit (EXEC_CMD ioctl) failed: errno %d",
          err);
    }
    int rc = hwq->wait_command(ebuf.get_exec_buf_bo(), 0);
    if (rc < 0) {
      ...
      return iree_make_status(
          IREE_STATUS_INTERNAL,
          "amdxdna reconfiguration wait ioctl failed: errno %d", -rc);
    }
    if (rc == 0) {
      IREE_TRACE_ZONE_END(z0);
      return iree_make_status(IREE_STATUS_DEADLINE_EXCEEDED,
                              "amdxdna control-packet reconfiguration timed "
                              "out");
    }
    ...
    if (ebuf.m_cmd_pkt->state != ERT_CMD_STATE_COMPLETED) {
      ...
      return iree_make_status(
          IREE_STATUS_INTERNAL,
          "amdxdna control-packet reconfiguration did not complete: ert "
          "state %u",
          ebuf.m_cmd_pkt->state);
    }
  }
...
static iree_status_t iree_hal_amdxdna_direct_command_buffer_dispatch(
  ...
  if (num_reconfigurations == 0) {
    ...
    executable->context = iree_hal_amdxdna_device_get_or_create_context(
