...
static iree_status_t iree_hal_amdxdna_direct_command_buffer_dispatch(
  ...
  iree_const_byte_span_t pdi_span =
      iree_make_const_byte_span(kernel_params.pdi.data(),
                                kernel_params.pdi.size());
  iree_string_view_t kernel_name =
      iree_make_string_view(kernel_params.kernel_name.data(),
                            kernel_params.kernel_name.size());
  std::shared_ptr<iree_hal_amdxdna_native_context_t> context;
  iree_hal_amdxdna_native_cu_index_t cu_idx;
  if (num_reconfigurations == 0) {
  ...
  if (command_buffer->device->cmd_chain) {
    ...
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0,
        iree_hal_amdxdna_direct_command_buffer_accumulate_chained(
