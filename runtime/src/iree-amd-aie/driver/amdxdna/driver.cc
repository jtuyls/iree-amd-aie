  ...
  *out_driver = nullptr;
  ...
  if (IREE_UNLIKELY(
          !iree_host_size_checked_add(total_size, identifier.size,
                                      &total_size) ||
          !iree_host_size_checked_add(total_size, device_params->device_path.size,
                                      &total_size) ||
          !iree_host_size_checked_add(total_size, device_params->power_mode.size,
                                      &total_size))) {
    IREE_TRACE_ZONE_END(z0);
  ...
  iree_string_view_append_to_buffer(
      identifier, &driver->identifier, string_storage);
